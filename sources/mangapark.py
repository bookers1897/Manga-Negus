...
class MangaParkConnector(BaseConnector):
    id = "mangapark"
    name = "MangaPark"
    ...
    url_patterns = [
        r'https?://(?:www\.)?mangapark\.(?:net|org|io|com)/manga/([a-z0-9_-]+)',  # e.g., /manga/naruto
    ]
    rate_limit = 2.5
    rate_limit_burst = 5
    request_timeout = 20
    ...
    def search(self, query: str, page: int = 1) -> List[MangaResult]:
        ...
        self._log(f"🔍 Searching MangaPark: {query}")
        html = self._request_html(f"{self.base_url}/search?word={quote(query)}")
        if not html: return []
        soup = BeautifulSoup(html, 'html.parser')
        results = []
        for item in soup.select('.item, .manga-item')[:20]:
            try:
    ...
    def get_chapters(self, manga_id: str, language: str = "en") -> List[ChapterResult]:
        if not HAS_BS4: return []
        self._log(f"📖 Fetching chapters from MangaPark...")
        url = f"{self.base_url}/comic/{manga_id}" if not manga_id.startswith('http') else manga_id
        html = self._request_html(url)
        if not html: return []
        soup = BeautifulSoup(html, 'html.parser')
        results = []
        for item in soup.select('.chapter-item, .episode-item, #chapterlist li'):
            try:
                link = item.select_one('a')
                if not link: continue
    ...
    def get_pages(self, chapter_id: str) -> List[PageResult]:
        ...
        soup = BeautifulSoup(html, 'html.parser')
        pages = []
        for i, img in enumerate(soup.select('.img-container img, .reader-img, .chapter-img')):
