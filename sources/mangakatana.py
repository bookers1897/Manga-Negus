"""MangaKatana Connector - Fast scraper with cloudscraper bypass"""
import re
...
class MangaKatanaConnector(BaseConnector):
    ...
    def search(self, query: str, page: int = 1) -> List[MangaResult]:
        ...
        self._log(f"🔍 Searching MangaKatana: {query}")
        html = self._request_html(f"{self.base_url}/?search={quote(query)}&search_by=book_name")
        if not html: return []
        soup = BeautifulSoup(html, 'html.parser')
        results = []
        for item in soup.select('#book_list .item')[:20]:
            try:
    ...
    def get_popular(self, page: int = 1) -> List[MangaResult]:
        if not HAS_BS4: return []
        html = self._request_html(f"{self.base_url}/manga/page/{page}?filter=1&include_mode=and&order=views")
        if not html: return []
        soup = BeautifulSoup(html, 'html.parser')
        results = []
        for item in soup.select('#book_list .item')[:20]:
    ...
    def get_chapters(self, manga_id: str, language: str = "en") -> List[ChapterResult]:
        if not HAS_BS4: return []
        self._log(f"📖 Fetching chapters from MangaKatana...")
        url = f"{self.base_url}/manga/{manga_id}" if not manga_id.startswith('http') else manga_id
        html = self._request_html(url)
        if not html: return []
        soup = BeautifulSoup(html, 'html.parser')
        results = []
        for item in soup.select('.chapters .chapter'):
            try:
                link = item.select_one('a')
                if not link: continue
    ...
    def get_pages(self, chapter_id: str) -> List[PageResult]:
        ...
        soup = BeautifulSoup(html, 'html.parser')
        pages = []
        for i, img in enumerate(soup.select('.wrap_img img, #imgs img')):
