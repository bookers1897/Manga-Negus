...
class MangaReaderConnector(BaseConnector):
    id = "mangareader"
    name = "MangaReader"
    ...
    url_patterns = [
        r'https?://(?:www\.)?mangareader\.to/manga/([a-z0-9_-]+)',  # e.g., /manga/naruto
    ]
    rate_limit = 2.0
    rate_limit_burst = 4
    request_timeout = 20
    ...
    def search(self, query: str, page: int = 1) -> List[MangaResult]:
        ...
        self._log(f"🔍 Searching MangaReader: {query}")
        html = self._request_html(f"{self.base_url}/search?keyword={quote(query)}")
        if not html: return []
        soup = BeautifulSoup(html, 'html.parser')
        results = []
        for item in soup.select('.manga_list-sbs .item, .film_list-wrap .flw-item')[:20]:
            try:
                link = item.select_one('a')
                if not link: continue
                url = urljoin(self.base_url, link.get('href', ''))
                title = link.get('title', '') or item.select_one('.film-name').get_text(strip=True)
                manga_id = url.rstrip('/').split('/')[-1]
                img = item.select_one('img')
                cover = urljoin(self.base_url, img.get('data-src', '') or img.get('src', '')) if img else None
    ...
    def get_popular(self, page: int = 1) -> List[MangaResult]:
        if not HAS_BS4: return []
        html = self._request_html(f"{self.base_url}/filter?sort=most_viewed&page={page}")
        if not html: return []
        soup = BeautifulSoup(html, 'html.parser')
        results = []
        for item in soup.select('.manga_list-sbs .item, .film_list-wrap .flw-item')[:20]:
    ...
    def get_chapters(self, manga_id: str, language: str = "en") -> List[ChapterResult]:
        if not HAS_BS4: return []
        self._log(f"📖 Fetching chapters from MangaReader...")
        url = f"{self.base_url}/{manga_id}" if not manga_id.startswith('http') else manga_id
        html = self._request_html(url)
        if not html: return []
        soup = BeautifulSoup(html, 'html.parser')
        results = []
        for item in soup.select('#chapters-list-1 .item, .chapter-item'):
            try:
                link = item.select_one('a')
                if not link: continue
    ...
    def get_pages(self, chapter_id: str) -> List[PageResult]:
        ...
        soup = BeautifulSoup(html, 'html.parser')
        pages = []
        for i, img in enumerate(soup.select('#main-wrapper img, .container-reader-chapter img')):
