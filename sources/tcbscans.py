"""TCB Scans Connector - Fast One Piece updates"""
import re
...
class TCBScansConnector(BaseConnector):
    id = "tcbscans"
    name = "TCB Scans"
    ...
    url_patterns = [
        r'https?://(?:www\.)?tcbscans\.(?:com|net|org)/mangas?/([a-z0-9_-]+)',  # e.g., /manga/naruto
    ]
    rate_limit = 2.5
    rate_limit_burst = 5
    request_timeout = 20
    ...
    def search(self, query: str, page: int = 1) -> List[MangaResult]:
        ...
        self._log(f"🔍 Searching TCB Scans: {query}")
        html = self._request_html(f"{self.base_url}/projects")
        if not html: return []
        soup = BeautifulSoup(html, 'html.parser')
        results = []
        query_lower = query.lower()
        for item in soup.select('.project-card, .manga-card')[:20]:
            try:
                link = item.select_one('a')
                if not link: continue
                url = urljoin(self.base_url, link.get('href', ''))
                title = link.get('title', '') or item.select_one('.title, h3').get_text(strip=True)
                if query_lower not in title.lower(): continue
    ...
    def get_chapters(self, manga_id: str, language: str = "en") -> List[ChapterResult]:
        if not HAS_BS4: return []
        self._log(f"📖 Fetching chapters from TCB Scans...")
        url = f"{self.base_url}/mangas/{manga_id}" if not manga_id.startswith('http') else manga_id
        html = self._request_html(url)
        if not html: return []
        soup = BeautifulSoup(html, 'html.parser')
        results = []
        for item in soup.select('.chapter-list a, .episode-item'):
            try:
                if not item.name == 'a':
                    link = item.select_one('a')
                else:
    ...
    def get_pages(self, chapter_id: str) -> List[PageResult]:
        ...
        soup = BeautifulSoup(html, 'html.parser')
        pages = []
        for i, img in enumerate(soup.select('.chapter-page img, .reader-img')):
