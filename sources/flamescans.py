...
class FlameScansConnector(BaseConnector):
    id = "flamescans"
    name = "FlameScans"
    ...
    url_patterns = [
        r'https?://(?:www\.)?flamescans\.org/series/([a-z0-9_-]+)',  # e.g., /series/naruto
    ]
    rate_limit = 2.0
    rate_limit_burst = 4
    request_timeout = 20
    ...
    def search(self, query: str, page: int = 1) -> List[MangaResult]:
        ...
        self._log(f"🔍 Searching FlameScans: {query}")
    ...
    def get_popular(self, page: int = 1) -> List[MangaResult]:
        if not HAS_BS4: return []
        html = self._request_html(f"{self.base_url}/series/?page={page}&status=&order=popular")
        if not html: return []
        soup = BeautifulSoup(html, 'html.parser')
        results = []
        for item in soup.select('.bsx, .listupd .bs')[:20]:
    ...
    def get_chapters(self, manga_id: str, language: str = "en") -> List[ChapterResult]:
        if not HAS_BS4: return []
        self._log(f"📖 Fetching chapters from FlameScans...")
